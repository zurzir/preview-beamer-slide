var RewrappingTools = {
    ...
    domyrewrap: function (joinonly) {
        ...
        if (hasSelection) {
            ...
            if (f.empty_line(e.get_line_text(fromLine))) {
                return false;
            }

            // no text selection present: search for start & end of paragraph
            while (fromLine > 0 &&
                !f.line_begins_with_item(e.get_line_text(fromLine)) &&
                !f.empty_line(e.get_line_text(fromLine - 1))) --fromLine;
            isitem = f.line_begins_with_item(e.get_line_text(fromLine));
            while (toLine < e.get_number_lines() - 1 &&
                !f.empty_line(e.get_line_text(toLine + 1)) &&
                (!isitem || !f.line_begins_with_item(e.get_line_text(toLine + 1)))) ++toLine;
        }
        ...
        if (isitem) {
            followIndent += f.get_item_extra_indent(e.get_line_text(fromLine));
        }

        // get text
        var lines = [];
        for (var i = fromLine; i <= toLine; i++)
            lines.push(e.get_line_text(i).trim());
        var text = lines.join(" ");

        // wrap lines
        var newlines;
        if (joinonly)
            newlines = [startIndent + text];
        else
